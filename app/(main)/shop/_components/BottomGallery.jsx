...
const MarqueeColumn = ({ images, speed = "medium", direction = "up", className = "", onImageClick }) => {
  ...
  return (
    <div className={`relative flex flex-col overflow-hidden h-[450px] sm:h-[500px] md:h-[550px] lg:h-[600px] rounded-[24px] bg-transparent ${className}`}>
      <div className={`flex flex-col gap-4 shrink-0 ${directionClass} ${speedClass} hover:[animation-play-state:paused]`}>
        {duplicatedImages.map((img, idx) => (
          <div 
            key={`${idx}`} 
            onClick={() => onImageClick && onImageClick(img)}
            className="relative aspect-[4/5] w-full overflow-hidden rounded-[24px] border border-white/5 group shadow-[0_4px_20px_rgba(0,0,0,0.4)] transition-all duration-300 hover:border-primary/20 cursor-pointer"
          >
            <Image
              src={img}
    ...
    <section className="relative w-full bg-[#050505] py-16 section-padding-x overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="w-full max-w-[800px] mx-auto z-10 relative select-none">
        <div className="grid grid-cols-3 gap-4 sm:gap-6 w-full">
          {/* Column 1: Scrolls Up */}
          <MarqueeColumn images={col1Images} speed="medium" direction="up" className="translate-y-4" onImageClick={onImageClick} />
          
          {/* Column 2: Scrolls Down */}
          <MarqueeColumn images={col2Images} speed="slow" direction="down" className="-translate-y-4" onImageClick={onImageClick} />
          
          {/* Column 3: Scrolls Up */}
          <MarqueeColumn images={col3Images} speed="fast" direction="up" className="translate-y-4" onImageClick={onImageClick} />
        </div>
      </div>
    </section>
  );
