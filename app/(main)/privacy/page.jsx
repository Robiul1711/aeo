...
import { useGetPrivacyPolicyCMSQuery } from '@/redux/api/apiSlice';
import bg from '@/assets/ebg.png';

const PrivacyPolicyPage = () => {
  const { data: response, isLoading, isError } = useGetPrivacyPolicyCMSQuery();
  ...
  return (
    <section 
      className="w-full min-h-screen bg-[#050505] py-28 section-padding-x border-t border-white/5" 
      style={{
      ...
      <div className="max-w-4xl mx-auto flex flex-col gap-10">
        ...
        {isLoading && (
          <div className="flex justify-center items-center py-20">
            <p className="text-white/60 font-outfit text-lg">Loading privacy policy...</p>
          </div>
        )}
        ...
        {!isLoading && !isError && pageData?.content?.description && (
          <div 
            ...
            className="font-outfit text-secondary-gray text-[15px] sm:text-[16px] leading-relaxed flex flex-col gap-4 [&>h2]:text-white [&>h2]:text-[22px] [&>h2]:font-semibold [&>h2]:font-outfit [&>h2]:mt-8 [&>h2]:mb-2 [&>p]:text-secondary-gray [&>p]:leading-relaxed [&>a]:text-primary [&>a]:underline [&>a]:hover:text-primary/80"
          />
        )}
