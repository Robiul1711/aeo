...
import { useGetEventBySlugQuery, useConfirmBookingCheckoutMutation } from "@/redux/api/apiSlice";
...
const CheckoutPage = ({ params, searchParams }) => {
  ...
  const { data: eventResponse, isLoading: eventLoading, isError: eventError } = useGetEventBySlugQuery(id);
  const event = eventResponse?.data;

  // Stripe Checkout Mutation
  const [confirmCheckout, { isLoading: isSubmitting }] = useConfirmBookingCheckoutMutation();
  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      const result = await confirmCheckout({ reference, body: payload }).unwrap();
      ...
      toast.error(err?.data?.message || "An error occurred during checkout. Please try again.");
    }
  };
  ...
  if (eventLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0D0D0D] text-white">
        <p className="text-white/60 font-outfit text-lg">Loading checkout details...</p>
      </div>
    );
  }
  ...
  if (eventError || !event) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0D0D0D] text-white gap-4">
        <p className="text-red-500/80 font-outfit text-lg">Failed to load checkout details.</p>
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-8">
        ...
        <div className="flex items-center justify-center gap-4 py-6 border-b border-white/5 select-none">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/60 font-outfit text-[13px] sm:text-[14px] font-medium">Select seats</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 flex flex-col gap-8">
              ...
              <div className="bg-[#111111] border border-white/5 rounded-[24px] p-6 sm:p-8 flex flex-col gap-6 shadow-md">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-white/60 font-outfit text-[13px] sm:text-[14px] select-none">Special Requests</label>
                  <textarea
