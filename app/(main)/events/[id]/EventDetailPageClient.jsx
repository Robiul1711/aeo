...
const EventDetailPageClient = ({ params }) => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen text-white relative pb-20 section-padding-x animate-pulse">
        {/* Hero Banner Skeleton */}
        <div className="pt-24 pb-8 flex flex-col justify-end min-h-[35vh] md:min-h-[45vh]">
          {/* Breadcrumbs Skeleton */}
          <div className="w-48 h-4 bg-white/10 rounded-md mb-4" />
          {/* Title Skeleton */}
          <div className="w-3/4 max-w-2xl h-12 md:h-16 bg-white/10 rounded-xl" />
        </div>
        ...
        <div className="py-8 grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-14">
          ...
          <div className="lg:col-span-2 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="h-20 bg-white/5 border border-white/5 rounded-[16px]" />
              <div className="h-20 bg-white/5 border border-white/5 rounded-[16px]" />
              <div className="h-20 bg-white/5 border border-white/5 rounded-[16px]" />
            </div>
            ...
            <div className="flex flex-col gap-3 mt-4">
              ...
              <div className="w-11/12 h-4 bg-white/5 rounded" />
              <div className="w-4/5 h-4 bg-white/5 rounded" />
            </div>

            {/* Banner Image Skeleton */}
            <div className="w-full aspect-video rounded-[24px] bg-white/5 border border-white/5 mt-4" />
          </div>

          {/* Right Column Sidebar Skeleton */}
          <div className="lg:col-span-1">
            <div className="bg-[#111111]/70 border border-white/5 rounded-[24px] p-6 sm:p-8 flex flex-col gap-6">
              <div className="w-32 h-6 bg-white/10 rounded-md" />
              <div className="h-24 bg-white/5 border border-white/5 rounded-[14px]" />
              <div className="h-12 bg-white/5 border border-white/5 rounded-[12px]" />
              <div className="h-14 bg-white/15 rounded-[12px] mt-2" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen text-white relative pb-20 section-padding-x">
      ...
      <div className=" py-8 grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-14">
        ...
        <div className="lg:col-span-2 flex flex-col">
          ...
          {/* About Section */}
          <div className="mt-12">
            <h2 className="text-white text-[20px] sm:text-[22px] font-outfit font-semibold tracking-wide">
              About This Event
            </h2>
          ...
          <div className="mt-6">
            ...
            <p className="text-secondary-gray text-[14px] font-outfit mt-1">
          ...
          {event.gallery && event.gallery.length > 0 && (
            <div className="mt-14">
              <h2 className="text-white text-[20px] sm:text-[22px] font-outfit font-semibold tracking-wide mb-6">
                Gallery
              </h2>
            ...
            {event.seat_categories && event.seat_categories.length > 0 ? (
              <div className="flex flex-col gap-3">
                {event.seat_categories.map((cat, idx) => {
                  ...
                  const serviceCharge =
                    (catPrice * cat.service_charge_pct) / 100;
                  const totalPerSeat = catPrice + serviceCharge;
                  return (
                    <div
                      key={cat.id}
                      className="border border-primary/20 bg-primary/5 rounded-[14px] p-4 flex flex-col gap-2 select-none"
                    >
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-white/70 font-outfit text-[13px] font-medium uppercase tracking-wider">
                            {event.seat_categories.length > 1
                              ? `Category ${idx + 1}`
                              : "General Admission"}
                          </span>
                        </div>
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-white/40 font-outfit text-[12px]">
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-white/60 font-outfit text-[13px]">
                          Total per seat
                        </span>
                        <span className="text-white font-outfit font-semibold text-[15px]">
              ...
              <div className="flex items-center justify-between bg-white/3 border border-white/5 rounded-[12px] px-4 py-3 select-none">
                ...
                <span
                  className={`font-outfit font-bold text-[15px] ${
                    event.seat_summary.available > 10
                      ? "text-[#00DF89]"
                      : event.seat_summary.available > 0
                        ? "text-yellow-400"
                        : "text-red-400"
                  }`}
                >
                  {event.seat_summary.available > 0
                    ? `${event.seat_summary.available} left`
                    : "Sold Out"}
                </span>
              </div>
            )}
